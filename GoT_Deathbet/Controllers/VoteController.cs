...
namespace GoT_Deathbet.Controllers
{
    public class VoteController : ApiController
    {
        ...
        public JsonResult<Candidate[]> Get_Duel()
        {
            ...
            int count = db.Table<Candidate>().CountAsync().Result;
            ...
            Ret[0] = db.Table<Candidate>().ElementAtAsync(right).Result;

            Ret[1] = db.Table<Candidate>().ElementAtAsync(left).Result;
        ...
        */
        ...
        public string Post_Result(int id, string Winner_id, string Loser_id)
        {
            try
            {
                ...
                SQLite.SQLiteAsyncConnection db = new SQLite.SQLiteAsyncConnection(database_name);
                var Winner = db.Table<Candidate>().Where(x => x.ID == Winner_guid).FirstOrDefaultAsync().Result;
                var Loser = db.Table<Candidate>().Where(x => x.ID == Loser_guid).FirstOrDefaultAsync().Result;
